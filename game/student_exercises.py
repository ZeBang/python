"""
Student Exercises - Stickman Defense Game
学生练习文件 - 火柴人防御游戏

这个文件包含了一系列练习任务，帮助学生逐步学习Python基础知识。
按照顺序完成这些练习，从简单到复杂。

使用方法：
1. 打开 stickman_defense.py
2. 找到对应的代码位置
3. 按照练习说明修改代码
4. 运行游戏，观察效果
"""

# ============================================================================
# 练习1：修改变量（最简单）
# ============================================================================
"""
任务：修改游戏配置，让游戏更容易

位置：stickman_defense.py 第44-49行

练习：
1. 将 PLAYER_SPEED 改为 10，让火柴人移动更快
2. 将 BULLET_SPEED 改为 15，让子弹更快
3. 将 MAX_MONSTERS_IN_HOME 改为 10，让游戏更容易

观察：运行游戏，看看这些改变如何影响游戏体验
"""

# ============================================================================
# 练习2：列表中嵌套元组：roof_points
# ============================================================================
"""
roof_points = [(...), (...), (...)]（251–255 行）
这是讲“列表 + 元组组合”的绝佳例子：
列表：存很多点 → 多个顶点的集合。
元组：每一个点 (x, y) 是不希望被修改的固定坐标。
"""


# ============================================================================
# 练习3：理解 if 语句
# ============================================================================
"""
任务：添加新的移动键

位置：stickman_defense.py 第72-108行，Stickman.update() 方法

练习：在 update() 方法中添加以下代码，让按 Q 键时向左上移动

# 在 # Vertical movement 部分之后添加：
if keys[pygame.K_q]:
    dx -= self.speed
    dy -= self.speed

理解：
- if 语句用于条件判断
- keys[pygame.K_q] 检查 Q 键是否被按下
- 如果条件为真，执行缩进的代码
"""

# ============================================================================
# 练习4：理解 if-else 语句
# ============================================================================
"""
任务：添加加速功能

位置：stickman_defense.py 第72-108行，Stickman.update() 方法

练习：修改移动代码，让按住 Shift 时移动更快

# 在 # Movement - support diagonal movement 之后添加：
speed_multiplier = 1  # 默认速度
if keys[pygame.K_LSHIFT]:  # 如果按住左Shift键
    speed_multiplier = 2  # 速度加倍
else:
    speed_multiplier = 1  # 正常速度

# 然后在应用移动时：
self.x += dx * speed_multiplier
self.y += dy * speed_multiplier

理解：
- if-else 用于两种情况的判断
- 如果条件为真执行 if 块，否则执行 else 块
"""

# ============================================================================
# 练习5：理解 for 循环
# ============================================================================
"""
任务：限制子弹数量

位置：stickman_defense.py 第114-136行，Stickman.shoot() 方法

练习：修改 shoot() 方法，限制最多只能有 10 发子弹

# 在 shoot() 方法开头添加：
# 注意：需要在 Game 类中传递 bullets 列表，或者使用其他方法
# 这里只是示例，实际需要在 Game.update() 中修改

理解：
- for 循环用于重复执行代码
- len() 函数返回列表的长度
- 可以用 if 语句检查条件
"""

# ============================================================================
# 练习6：理解 while 循环
# ============================================================================
"""
任务：理解游戏主循环

位置：stickman_defense.py 第588-628行，Game.run() 方法

观察：理解这段代码的结构

while self.running:  # 只要游戏运行，就重复执行
    # 1. 处理事件（键盘、鼠标输入）
    # 2. 更新游戏状态
    # 3. 绘制画面
    # 4. 控制帧率

理解：
- while 循环会一直执行，直到条件为假
- 游戏循环是游戏的核心
- 每帧执行一次更新和绘制
"""

# ============================================================================
# 练习7：创建函数
# ============================================================================
"""
任务：创建一个计算距离的函数

位置：可以在文件开头（类定义之前）添加

练习：创建以下函数

def calculate_distance(x1, y1, x2, y2):
    \"\"\"
    计算两点之间的距离
    参数：
    - x1, y1: 第一个点的坐标
    - x2, y2: 第二个点的坐标
    返回：两点之间的距离
    \"\"\"
    dx = x2 - x1
    dy = y2 - y1
    distance = math.sqrt(dx*dx + dy*dy)
    return distance

然后在碰撞检测中使用这个函数：
# 在 Game.update() 方法中，找到碰撞检测部分
# 将原来的距离计算替换为：
distance = calculate_distance(bullet.x, bullet.y, monster.x, monster.y)

理解：
- 函数用于组织代码，避免重复
- 参数是函数的输入
- return 是函数的输出
"""

# ============================================================================
# 练习8：修改类的方法
# ============================================================================
"""
任务：给火柴人添加能量系统

位置：stickman_defense.py 第59-136行，Stickman 类

练习1：添加能量属性
# 在 __init__() 方法中添加：
self.energy = 100
self.max_energy = 100

练习2：创建使用能量的方法
# 在 Stickman 类中添加新方法：
def use_energy(self, amount):
    \"\"\"使用能量\"\"\"
    if self.energy >= amount:
        self.energy -= amount
        return True
    return False

练习3：修改射击，消耗能量
# 在 shoot() 方法中，添加能量检查：
if not self.use_energy(10):
    return None  # 能量不足，不能射击

理解：
- 类的方法可以访问和修改对象的属性
- self 代表对象本身
- 方法可以返回值
"""

# ============================================================================
# 练习9：列表操作
# ============================================================================
"""
任务：添加不同类型的怪物

位置：stickman_defense.py 第409-412行，Game.spawn_monster() 方法

练习：修改 spawn_monster() 方法，随机生成不同类型的怪物

def spawn_monster(self):
    y = random.randint(50, SCREEN_HEIGHT - 50)
    monster_type = random.choice(['normal', 'fast', 'slow'])
    
    monster = Monster(SPAWN_X, y)
    
    if monster_type == 'fast':
        monster.speed = 5
        monster.color = RED
    elif monster_type == 'slow':
        monster.speed = 1
        monster.color = BLUE
    else:
        monster.speed = 3
        monster.color = ORANGE
    
    self.monsters.append(monster)

理解：
- random.choice() 从列表中随机选择一个元素
- if-elif-else 用于多个条件的判断
- 列表的 append() 方法用于添加元素
"""

# ============================================================================
# 练习10：综合练习 - 添加分数系统
# ============================================================================
"""
任务：创建更复杂的分数系统

位置：stickman_defense.py 第414-500行，Game.update() 方法

练习：根据怪物类型给予不同分数

# 在击中怪物后，修改分数计算：
if distance < bullet.size + monster.size:
    # 根据怪物速度给予不同分数
    if monster.speed >= 4:
        self.score += 20  # 快速怪物给更多分
    elif monster.speed <= 2:
        self.score += 5   # 慢速怪物给较少分
    else:
        self.score += 10  # 普通怪物
    
    self.bullets.remove(bullet)
    self.monsters.remove(monster)
    break

理解：
- 综合运用 if-elif-else
- 根据条件执行不同的操作
- 理解游戏逻辑的实现
"""

# ============================================================================
# 练习11：挑战题 - 添加连击系统
# ============================================================================
"""
任务：创建连击系统（高级）

位置：stickman_defense.py 第381-407行，Game.__init__() 方法

练习：
1. 在 __init__() 中添加连击相关变量：
   self.combo = 0
   self.combo_timer = 0
   self.max_combo = 0

2. 在 update() 方法中更新连击：
   # 在击中怪物后：
   self.combo += 1
   self.combo_timer = 180  # 3秒内必须再次击中
   
   # 在 update() 开头：
   if self.combo_timer > 0:
       self.combo_timer -= 1
   else:
       if self.combo > self.max_combo:
           self.max_combo = self.combo
       self.combo = 0

3. 在 draw_ui() 中显示连击：
   if self.combo > 0:
       combo_text = self.font_small.render(f"Combo: {self.combo}x", True, YELLOW)
       self.screen.blit(combo_text, (10, 170))

理解：
- 综合运用所有学过的概念
- 理解游戏系统的设计
- 学会添加新功能
"""

# ============================================================================
# 学习检查清单
# ============================================================================
"""
完成这些练习后，检查你是否理解：

□ 变量和常量
  - 知道如何修改变量的值
  - 理解变量对游戏的影响

□ 条件语句 (if/else)
  - 能够使用 if 语句做判断
  - 理解 if-elif-else 的使用
  - 能够添加新的条件逻辑

□ 循环 (for/while)
  - 理解 for 循环遍历列表
  - 理解 while 循环的作用
  - 知道如何控制循环

□ 函数
  - 能够创建简单的函数
  - 理解参数和返回值
  - 知道如何调用函数

□ 类和对象
  - 理解类的基本概念
  - 知道如何添加属性和方法
  - 理解 self 的作用

□ 列表操作
  - 知道如何添加和删除元素
  - 理解列表的遍历
  - 能够使用列表管理游戏对象

如果都理解了，恭喜你！你已经掌握了Python的基础知识！🎉
"""
